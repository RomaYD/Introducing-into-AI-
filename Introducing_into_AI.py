...
data = pd.read_excel('DataTable1.xlsx')
...
cluster = cluster_vectoriz.fit_transform(data['Cluster'])
...
model = MLPClassifier(hidden_layer_sizes=(128), activation='tanh', solver='adam', max_iter=10, early_stopping=True, verbose=True)
model.fit(word_train, cluster_train)

# Оценка производительности
score = model.score(word_test, cluster_test)
print('Точность модели accurose:', score)

# Сохранение модели
joblib.dump(model, 'new_dat.pkl')
